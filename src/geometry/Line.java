 ...
 * Provides methods to calculate length, check for intersection, and determine other properties of the line.
 */
public class Line {
    // Fields
    ...
    /**
     * Constructs a shapes.Line given four coordinates (x1, y1) for the start point and (x2, y2) for the end point.
     *
    ...
    public boolean onSegment(Point p) {
        return p.getX() >= Math.min(this.start.getX(), this.end.getX()) - EPSILON && p.getX()
                <= Math.max(this.start.getX(), this.end.getX())
                + EPSILON && p.getY() >= Math.min(this.start.getY(), this.end.getY())
                - EPSILON && p.getY() <= Math.max(this.start.getY(), this.end.getY()) + EPSILON;
    }
    ...
    public Point intersectionWith(Line other) {
        ...
        if (det == 0) {
            ...
            if (this.onSegment(other.start)
                ...
                return this.start; // Or you can choose to return other.start or any other point on the segments
            }
            return null;
        }
