...
/**
 * The sprites.Ball class represents a ball with a center point, radius, and color. It provides
 * methods to access its properties and draw it on a given surface.
 */
public class Ball implements Sprite {
  ...
  /**
   * Adds this sprite to the specified game. The sprite will be managed by the game and be part of
   * the game's sprite collection, allowing it to be drawn and updated during the game loop.
   *
   * @param g the game to which this sprite will be added. This game will manage the sprite and
   *     include it in its sprite collection.
   */
   ...
   * The trajectory is represented as a line from the ball's current center to the point where it
   * would move next if no collisions occur.
   *
  ...
  public Line getTrajectory() {
    ...
    Point end =
        new Point(center.getX() + velocity.getDx() * 3, center.getY() + velocity.getDy() * 3);
  ...
  /**
   * Moves the ball one step forward based on its current velocity. The method updates the ball's
   * position by applying its velocity to the current position. The new position is computed by
   * calling the {@link Velocity#applyToPoint(Point)} method on the current position.
   *
  ...
  public void timePassed() {
    ...
    if (collisionInfo == null) {
      ...
      Velocity newVelocity =
          collisionInfo.collisionObject().hit(this, collisionPoint, this.getVelocity());
