...
public class GameEnvironment {
  ...
  /**
   * Assume an object moving from line.start() to line.end(). If this object will not collide with
   * any of the collidables in this collection, returns null. Else, returns the information about
  ...
  public CollisionInfo getClosestCollision(Line trajectory) {
    ...
    for (Collidable collidable : collidables) {
      ...
      if (p != null) {
        // If closestPoint is null (first valid collision) or if p is closer than the previous
        // closest
        if (closestPoint == null
            || trajectory.getStart().distance(p) < trajectory.getStart().distance(closestPoint)) {
          closestPoint = p;
